...
import utils
import dataloader
from transformer_model import TransformerEncoderDecoder
...
class TransformerRunner:
    ...
    def save_model(self):
        ...
        self.model.cpu()
        with utils.smart_open(self.opts.model_path, "wb") as model_file:
    ...
    def init_visualization(self):
        ...
        if self.opts.viz_tensorboard:
            ...
            writer = SummaryWriter(comment="run")  # outputs to ./runs/
        return writer
    ...
    def train(
        ...
        scheduler = torch.optim.lr_scheduler.LambdaLR(
            optimizer=optimizer,
            lr_lambda=lambda step: utils.schedule_rate(
    ...
    def train_input_target_split(
        ...
        # The input sequence for the model excludes the last token of the target sequence
        input_tokens = target_tokens[:, :-1]
        
        # The target sequence for training excludes the first token of the target sequence
    ...
    def train_for_epoch(
        ...
        for step, (source, source_lens, targets) in enumerate(dataloader):
            ...
            # Split the targets into input and target for the model
            target_input, target_output = self.train_input_target_split(targets) # [batch_size, tgt_seq_len - 1], [batch_size * (tgt_seq_len - 1)]

            # Forward pass
            logits = self.model(source, target_input) # [batch_size, tgt_seq_len - 1, tgt_vocab_size]
            
            # Compute loss
            loss = criterion(logits.reshape(-1, logits.size(-1)), target_output)
            ...
            total_loss += loss.item() * accum_iter  # Undo the scaling
            total_steps += 1

        average_loss = total_loss / total_steps

        return average_loss, total_steps
    ...
    def translate(self, input_sentence):
        ...
        source_tokens = [self.dataset.source_word2id.get(token, self.dataset.source_unk_id) for token in tokens]
        ...
        hypotheses_ids = hypotheses[0].tolist()
        translated_sentence = ' '.join([self.dataset.target_id2word[id] for id in hypotheses_ids])
    ...
    def compute_batch_total_bleu(
        ...
        total_scores = {level: 0.0 for level in n_gram_levels}
        ...
        for i in range(batch_size):
            ...
            # Remove all instances of special tokens from both sequences
            ref_clean = [str(tok) for tok in ref if tok not in special_tokens]
            cand_clean = [str(tok) for tok in cand if tok not in special_tokens]

            # Compute BLEU score for each n-gram level
            for level in n_gram_levels:
                score = bleu_score_func(ref_clean, cand_clean, level)
                total_scores[level] += score

        # Prepare the return values: summed BLEU scores for each n-gram level, and batch size
        summed_scores = list(total_scores[level] for level in n_gram_levels)
        return summed_scores, batch_size
